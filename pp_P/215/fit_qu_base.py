    ...
    def find_nearby_ps(self, num_ps=1, threshold_extra_factor=1.1):
        ...
        for i in range(min(num_ps, len(index_near[0]))):
            ...
            if index < self.df_mask.at[self.flux_idx, 'flux_idx']:
                ...
                qflux = self.flux2norm_beam(self.df_ps.at[index, 'qflux'])
                uflux = self.flux2norm_beam(self.df_ps.at[index, 'uflux'])
            else:
                ...
                qflux = self.flux2norm_beam(self.df_ps.at[index + 1, 'qflux'])
                uflux = self.flux2norm_beam(self.df_ps.at[index + 1, 'uflux'])
            lon_list.append(lon)
            lat_list.append(lat)
    ...
    def fit_all(self, cov_mode:str, mode:str='pipeline'):
        ...
        def lsq_params(*args):
            ...
            for i in range(num_ps):
                ...
                profile = 1 / (2 * np.pi * self.sigma**2) * np.exp(- (theta)**2 / (2 * self.sigma**2))
                P = (q_amp + 1j * u_amp) * np.exp(2j * np.deg2rad(lon))
                lugwid_P = P * profile
                QU = lugwid_P * np.exp(-2j * self.phi_around)
                Q = QU.real + c_q
                U = QU.imag + c_u
        ...
        def fit_1_ps():
            ...
            obj_minuit.limits = [(-1,1),(-1,1), (-500,500), (-500,500)]
            logger.debug(f'\n{obj_minuit.migrad()}')
            logger.debug(f'\n{obj_minuit.hesse()}')
        ...
        def fit_2_ps():
            ...
            obj_minuit.limits = [(-1,1),(-1,1),(-1,1),(-1,1),(-500,500), (-500,500)]
            logger.debug(f'\n{obj_minuit.migrad()}')
            logger.debug(f'\n{obj_minuit.hesse()}')
        ...
        if mode == 'pipeline':
            ...
            #         chi2dof, fit_q_amp, fit_q_amp_err, fit_u_amp, fit_u_amp_err = fit_8_ps()
            # except ValueError:
            #     logger.info('there might be some hesse error, try to use smaller threshold_extra_factor: 0.8')
            #     num_ps, near = self.find_nearby_ps(num_ps=10, threshold_extra_factor=0.8)
            #     logger.info(f'{num_ps=}, {near=}')
            #     try:
            #         if num_ps == 0:
            #             chi2dof, fit_q_amp, fit_q_amp_err, fit_u_amp, fit_u_amp_err = fit_1_ps()
            ...
            #             chi2dof, fit_q_amp, fit_q_amp_err, fit_u_amp, fit_u_amp_err = fit_8_ps()
            #     except ValueError:
            #         logger.info('there might be some hesse error, try to use smaller threshold_extra_factor: 0.5')
            #         num_ps, near = self.find_nearby_ps(num_ps=10, threshold_extra_factor=0.5)
            #         logger.info(f'{num_ps=}, {near=}')
            #         if num_ps == 0:
            #             chi2dof, fit_q_amp, fit_q_amp_err, fit_u_amp, fit_u_amp_err = fit_1_ps()
            #         elif num_ps == 1:
