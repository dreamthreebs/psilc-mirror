...
class FitPolPS:
    ...
    def __init__(self, m_i, m_q, m_u, freq, nstd_i, nstd_q, nstd_u, flux_idx, df_mask, df_ps, cl_cmb, lmax, nside, radius_factor, beam, sigma_threshold=5, epsilon=1e-4, debug_flag=False):
        self.m_i = m_i # sky maps (npix,)
        ...
        self.nstd_i = nstd_i # noise standard deviation of I
    ...
    def calc_definite_fixed_cmb_cov(self):
        ...
        cmb_cov_path = Path(f'./Cov_IQU.npy')
    ...
    def calc_covariance_matrix(self, mode='cmb+noise'):
        ...
        if mode == 'noise':
            nstd_i2 = (self.nstd_i**2)[self.ipix_fit].copy()
            nstd_q2 = (self.nstd_q**2)[self.ipix_fit].copy()
            nstd_u2 = (self.nstd_u**2)[self.ipix_fit].copy()
            nstd2 = np.concatenate([nstd_i2, nstd_q2, nstd_u2])
    ...
    def find_nearby_ps(self, num_ps=1):
        ...
        for i in range(min(num_ps, len(index_near[0]))):
            ...
            if index < self.df_mask.at[self.flux_idx, 'flux_idx']:
                ...
                pflux = self.flux2norm_beam(self.df_ps.at[index, 'pflux'])
            else:
                lon = np.rad2deg(self.df_ps.at[index + 1, 'lon'])
                lat = np.rad2deg(self.df_ps.at[index + 1, 'lat'])
                pflux = self.flux2norm_beam(self.df_ps.at[index + 1, 'pflux'])
            lon_list.append(lon)
            lat_list.append(lat)
        ...
        num_ps = np.count_nonzero(np.where(pflux_arr > self.flux2norm_beam(flux=1), pflux_arr, 0))
        ...
        return num_ps, tuple(sum(zip(pflux_arr, lon_arr, lat_arr), ()))
    ...
    def fit_all(self, cov_mode:str, mode:str='pipeline'):
        ...
        def lsq_2_params(i_amp, q_amp, u_amp, c_i, c_q, c_u):
            ...
            def model():
                ...
                I = i_amp / (2 * np.pi * self.sigma**2) * np.exp(- (theta)**2 / (2 * self.sigma**2)) +c_i
                Q = QU.real + c_q
                U = QU.imag + c_u
                return np.concatenate([I,Q,U])

            y_model = model()
            y_data = np.concatenate([self.m_i[ipix_fit], self.m_q[ipix_fit], self.m_u[ipix_fit]])
        ...
        def test_fit():
            params = (self.i_amp, self.q_amp, self.u_amp, 0, 0, 0)
            obj_minuit = Minuit(lsq_2_params, name=("i_amp", "q_amp", "u_amp", "c_i", "c_q", "c_u"), *params)
            obj_minuit.limits = [(-10,10), (-10,10),(-10,10),(-1000,1000),(-100,100),(-100,100)]
            logger.debug(f'\n{obj_minuit.migrad()}')
            logger.debug(f'\n{obj_minuit.hesse()}')
            ...
            return chi2dof, obj_minuit.values['i_amp'],obj_minuit.errors['i_amp'], obj_minuit.values['q_amp'],obj_minuit.errors['q_amp'], obj_minuit.values['u_amp'], obj_minuit.errors['u_amp']
        ...
        def fit_2_params():
            ...
            obj_minuit = Minuit(lsq_params, name=("q_amp_1","u_amp_1","const"), *params)
        ...
        if mode == 'pipeline':
            ...
            chi2dof, fit_i_amp, fit_i_amp_err, fit_q_amp, fit_q_amp_err, fit_u_amp, fit_u_amp_err = test_fit()
            # chi2dof, fit_q_amp, fit_q_amp_err, fit_u_amp, fit_u_amp_err = fit_2_params()
            ...
            fit_error_i = np.abs((fit_i_amp - self.i_amp) / self.i_amp )
            ...
            logger.info(f'{self.i_amp=}, {fit_i_amp=}, {fit_error_i=}')
...
def main():
    ...
    # m_i = np.load(f'../../fitdata/synthesis_data/2048/PSNOISE/{freq}/88.npy')[0].copy()
    # m_q = np.load(f'../../fitdata/synthesis_data/2048/PSNOISE/{freq}/88.npy')[1].copy()
    # m_u = np.load(f'../../fitdata/synthesis_data/2048/PSNOISE/{freq}/88.npy')[2].copy()

    m_i = np.load(f'../../fitdata/synthesis_data/2048/PSCMBNOISE/{freq}/2.npy')[0].copy()
    m_q = np.load(f'../../fitdata/synthesis_data/2048/PSCMBNOISE/{freq}/2.npy')[1].copy()
    m_u = np.load(f'../../fitdata/synthesis_data/2048/PSCMBNOISE/{freq}/2.npy')[2].copy()
    ...
    nstd_i = np.load(f'../../FGSim/NSTDNORTH/2048/{freq}.npy')[0].copy()
    ...
    logger.debug(f'{sys.getrefcount(m_q)-1=}')
    obj = FitPolPS(m_i=m_i, m_q=m_q, m_u=m_u, freq=freq, nstd_i=nstd_i, nstd_q=nstd_q, nstd_u=nstd_u, flux_idx=flux_idx, df_mask=df_mask, df_ps=df_ps, cl_cmb=cl_cmb, lmax=lmax, nside=nside, radius_factor=1.5, beam=beam, epsilon=0.00001)
