...
class FitPolPS:
    ...
    def __init__(self, m_q, m_u, freq, nstd, flux_idx, df_mask, df_ps, cl_cmb, lmax, nside, radius_factor, beam, sigma_threshold=5, epsilon=1e-4, debug_flag=False):
        self.m_q = m_q # sky maps (npix,)
        self.m_u = m_u # sky maps (npix,)
        self.freq = freq # frequency
        self.df_mask = df_mask # pandas data frame of point sources in mask
        self.lon = np.rad2deg(df_mask.at[flux_idx, 'lon'])
        self.lat = np.rad2deg(df_mask.at[flux_idx, 'lat'])
...
def main():
    ...
    m_q = np.load(f'../../fitdata/synthesis_data/2048/PSCMBNOISE/{freq}/0.npy')[0].copy()
    m_u = np.load(f'../../fitdata/synthesis_data/2048/PSCMBNOISE/{freq}/0.npy')[2].copy()
    ...
    logger.debug(f'{sys.getrefcount(m_q)-1=}')
    obj = FitPolPS(m_q=m_q, m_u=m_u, freq=freq, nstd=nstd, flux_idx=flux_idx, df_mask=df_mask, df_ps=df_ps, cl_cmb=cl_cmb, lmax=lmax, nside=nside, radius_factor=1.5, beam=beam, epsilon=0.00001)
