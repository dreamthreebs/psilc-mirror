...
class FitPolPS:
    ...
    def fit_all(self, cov_mode:str, mode:str='pipeline'):
        ...
        def lsq_2_params(i_amp, q_amp, u_amp):
            ...
            def model():
                ...
                I = i_amp / (2 * np.pi * self.sigma**2) * np.exp(- (theta)**2 / (2 * self.sigma**2))
                Q = QU.real
                U = QU.imag
        ...
        def test_fit():
            params = (self.i_amp, self.q_amp, self.u_amp)
            obj_minuit = Minuit(lsq_2_params, name=("i_amp", "q_amp", "u_amp"), *params)
            obj_minuit.limits = [(-10,10), (-10,10),(-10,10)]
            logger.debug(f'\n{obj_minuit.migrad()}')
            logger.debug(f'\n{obj_minuit.hesse()}')
