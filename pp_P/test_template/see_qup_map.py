...
class FitPointSource:
    ...
    def see_true_map(self, m, lon, lat, nside, beam):
        ...
        hp.gnomview(irps, rot=[lon, lat, 0], xsize=200, ysize=200, reso=1, title='irps')
        hp.gnomview(radiops, rot=[lon, lat, 0], xsize=200, ysize=200, reso=1, title='radiops')
        hp.gnomview(m, rot=[lon, lat, 0], xsize=200, ysize=200)
        plt.show()

        vec = hp.ang2vec(theta=lon, phi=lat, lonlat=True)
        ipix_disc = hp.query_disc(nside=nside, vec=vec, radius=np.deg2rad(beam)/200)

        mask = np.ones(hp.nside2npix(nside))
        mask[ipix_disc] = 0

        hp.gnomview(mask, rot=[lon, lat, 0], xsize=200, ysize=200, reso=1)
        plt.show()
...
def main():
    ...
    m_q = np.load(f'../../fitdata/2048/PS/40/ps.npy')[1]
    m_u = np.load(f'../../fitdata/2048/PS/40/ps.npy')[2]
    # m = np.sqrt(m_q**2 + m_u**2)
    m = m_u
    ...
    df_mask = pd.read_csv(f'../mask/ps_csv/{freq}.csv')
