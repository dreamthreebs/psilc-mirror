...
ps_mask = np.load(f'./inpaint_pcfn/{threshold}sigma/apo_mask/apo_c1_1/{rlz_idx}.npy')
...
def cpr_spectrum_pcfn_b(bin_mask, apo_mask):
    ...
    m_cfn = np.load(f'../../../../fitdata/synthesis_data/2048/CMBFGNOISE/{freq}/{rlz_idx}.npy')
    m_pcfn = np.load(f'../../../../fitdata/synthesis_data/2048/PSCMBFGNOISE/{freq}/{rlz_idx}.npy')

    m_cfn_b = hp.alm2map(hp.map2alm(m_cfn, lmax=lmax)[2], nside=nside) * bin_mask
    m_pcfn_b = hp.alm2map(hp.map2alm(m_pcfn, lmax=lmax)[2], nside=nside) * bin_mask

    m_removal_b = np.load(f'./pcfn_after_removal/{threshold}sigma/B/map_cln_b{rlz_idx}.npy')

    m_ps_b = hp.read_map(f'./inpaint_pcfn/{threshold}sigma/EB/B_input/{rlz_idx}.fits') * bin_mask
    m_inp_eb_b = hp.read_map(f'./inpaint_pcfn/{threshold}sigma/EB/B_output/{rlz_idx}.fits') * bin_mask
    # m_inp_qu_b = hp.read_map(f'./inpaint_pcn/{threshold}sigma/QU/B/{rlz_idx}.fits') * bin_mask

    b_min = -10
    b_max = 10
    # ### test: checking map
    hp.orthview(m_cfn_b, rot=[100,50,0], half_sky=True, title=' cfn b ', min=b_min, max=b_max)
    hp.orthview(m_pcfn_b, rot=[100,50,0], half_sky=True, title=' pcfn b ', min=b_min, max=b_max)
...
def cpr_spectrum_pcfn_e(bin_mask, apo_mask):
...
def main():
    ...
    cpr_spectrum_pcfn_b(bin_mask=bin_mask, apo_mask=apo_mask)
    # cpr_spectrum_pcfn_e(bin_mask=bin_mask, apo_mask=apo_mask)
