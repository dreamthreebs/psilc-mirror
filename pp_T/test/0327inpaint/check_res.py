...
inpaint_res2 = hp.read_map('./fits_file/inpaint_denoise.fits')
...
plt.plot(l*(l+1)*cl_inpaint1/(2*np.pi)/bl**2, label='dl inpaint diffuse')
plt.plot(l*(l+1)*cl_inpaint2/(2*np.pi)/bl**2, label='dl inpaint denoise')
plt.plot(l*(l+1)*cl_inpaint/(2*np.pi)/bl**2, label='dl inpaint ps_cmb_noise')
plt.semilogy()
plt.legend()
plt.xlim(2,2000)
plt.ylim(1e2, 1e6)
