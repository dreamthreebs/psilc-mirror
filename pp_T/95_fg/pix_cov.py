...
def test_fg_cl():
    cl_fg = np.load('../../fit_b/benchmark_T_95GHz_3sigma/data/debeam_full_b/cl_fg.npy')
    Cl_TT = cl_fg[0]
    Cl_EE = cl_fg[1]
    Cl_BB = cl_fg[2]
...
def main_cmb():
    ...
    Cl_TT,Cl_EE,Cl_BB,Cl_TE = test_cmb_cl(beam=beam, lmax=lmax)
    ...
    # obj = CovCalculator(nside=nside, lmin=2, lmax=lmax, Cl_TT=Cl_TT, Cl_EE=Cl_EE, Cl_BB=Cl_BB, Cl_TE=Cl_TE, pixind=pixind, calc_opt='polarization')
    obj = CovCalculator(nside=nside, lmin=2, lmax=lmax, Cl_TT=Cl_TT, Cl_EE=Cl_EE, Cl_BB=Cl_BB, Cl_TE=Cl_TE, pixind=pixind)
    MP = obj.run_calc_cov()
    path_test_class_cov = Path('./cmb_cov_1k6')
...
def main_fg():
    ...
    Cl_cmb,_,_,_ = test_cmb_cl(beam=beam, lmax=lmax)
    Cl_fg,_,_,_ = test_fg_cl()

    Cl_TT = Cl_cmb + Cl_fg

    l = np.arange(lmax + 1)
    plt.loglog(l, l*(l+1)*Cl_cmb/(2*np.pi), label='cmb')
    plt.loglog(l, l*(l+1)*Cl_fg/(2*np.pi), label='fg')
    plt.loglog(l, l*(l+1)*Cl_TT/(2*np.pi), label='cmb + fg')
    plt.legend()
    plt.show()
    ...
    obj = CovCalculator(nside=nside, lmin=2, lmax=lmax, Cl_TT=Cl_TT, Cl_EE=None, Cl_BB=None, Cl_TE=None, pixind=pixind)
    MP = obj.run_calc_cov()
    path_test_class_cov = Path('./cmb_fg_cov')
    path_test_class_cov.mkdir(exist_ok=True, parents=True)
    # np.save(f'./test_class_cov/cmb.npy', MP)
    np.save(f'./cmb_fg_cov/{flux_idx}.npy', MP)
