...
def gen_ps_remove_map_pcn(rlz_idx, mask):
    ...
    m_cmb_noise = np.load('../../../../fitdata/synthesis_data/2048_bak/CMBNOISE/155_bak/0.npy')[0].copy()
...
def cpr_spectrum_pcn(bin_mask, apo_mask, bl):
    ...
    # m_inpaint = hp.read_map('./for_inpainting/output/pcn/2sigma/0.fits', field=0) * bin_mask
    m_inpaint = hp.read_map('./for_inpainting/test_inpaint/100.fits', field=0) * bin_mask
    ...
    m_ps_cmb_noise = np.load('../../../../fitdata/synthesis_data/2048_bak/PSCMBNOISE/155/0.npy')[0]
    # dl_ps_cmb_noise = calc_dl_from_scalar_map(m_ps_cmb_noise, bl, apo_mask, bin_dl)

    m_cmb_noise = np.load('../../../../fitdata/synthesis_data/2048_bak/CMBNOISE/155_bak/0.npy')[0]
    # dl_cmb_noise = calc_dl_from_scalar_map(m_cmb_noise, bl, apo_mask, bin_dl)

    # hp.orthview((m_inpaint - m_cmb_noise) * bin_mask, rot=[100,50,0], half_sky=True)
    # plt.show()

    # delta_dl_ps = dl_ps_cmb_noise - dl_cmb_noise
    # delta_dl_inpaint = dl_inpaint - dl_cmb_noise
    # delta_dl_removal = dl_removal - dl_cmb_noise

    # m_cmb = np.load('../../../../fitdata/2048/CMB/155/0.npy')[0]
    # dl_cmb = calc_dl_from_scalar_map(m_cmb, bl, apo_mask, bin_dl)

    # m_ps = np.load('../../../../fitdata/2048/PS/155/ps.npy')[0]
    ...
    # m_inpaint_res = gen_inpaint_res_map_pcn(rlz_idx=0, mask=bin_mask)
    # dl_inpaint_res = calc_dl_from_scalar_map(m_inpaint_res, bl, apo_mask, bin_dl)
    ...
    # plt.plot(ell_arr, dl_inpaint_res, label='ps inpaint res', marker='o')

    # plt.plot(ell_arr, delta_dl_ps, label='ps', marker='o')
    # plt.plot(ell_arr, delta_dl_inpaint, label='inpaint', marker='o')
    # plt.plot(ell_arr, delta_dl_removal, label='removal', marker='o')
    # # plt.semilogy()
