...
df = pd.read_csv('../../../../mask/mask_csv/155.csv')
lon = np.rad2deg(df.at[flux_idx,'lon'])
lat = np.rad2deg(df.at[flux_idx,'lat'])

hp.gnomview(m_avg, rot=[lon, lat, 0], title=f'ps_res avg, {flux_idx=}', xsize=30,ysize=30)
hp.gnomview(m_var, rot=[lon, lat, 0], title=f'ps_res var, {flux_idx=}', xsize=30,ysize=30)
hp.gnomview(m_one_rlz, rot=[lon, lat, 0], title=f'ps_res one rlz, {flux_idx=}', xsize=30,ysize=30)
plt.show()
