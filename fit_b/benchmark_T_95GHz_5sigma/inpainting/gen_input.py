...
# m_pcfn_b = hp.alm2map(hp.map2alm(m_pcfn)[2], nside=2048)
# m_n_b = hp.alm2map(hp.map2alm(m_n)[2], nside=2048)
...
crt_pcfn,_,cln_b_pcfn = obj.run_eblc()

obj = EBLeakageCorrection(m_n, lmax=lmax, nside=nside, mask=mask, post_mask=mask)
crt_n,_,cln_b_n = obj.run_eblc()

path_input = Path('./test_1108_pcfn_edge')
path_input.mkdir(exist_ok=True, parents=True)
hp.write_map(path_input / Path(f'{rlz_idx}.fits'), cln_b_pcfn, overwrite=True)

path_input = Path('./test_1108_n_edge')
path_input.mkdir(exist_ok=True, parents=True)
hp.write_map(path_input / Path(f'{rlz_idx}.fits'), cln_b_n, overwrite=True)


# path_input = Path('./test_1101')
# path_input.mkdir(exist_ok=True, parents=True)
# hp.write_map(path_input / Path(f'{rlz_idx}.fits'), cln_b_pcfn, overwrite=True)

# path_input = Path('./input_1031_n')
# path_input.mkdir(exist_ok=True, parents=True)
# hp.write_map(path_input / Path(f'{rlz_idx}.fits'), cln_b_n, overwrite=True)
