...
def cpr_spectrum_pcn_b(bin_mask, apo_mask):
    ...
    # m_removal_b_qu = np.load(f'./pcn_fit_b_qu_1/{threshold}sigma/B/{rlz_idx}.npy') * bin_mask
    ...
    path_dl_c = Path(f'pcn_dl/QU_B_1/c')
    path_dl_c.mkdir(parents=True, exist_ok=True)
    path_dl_cn = Path(f'pcn_dl/QU_B_1/cn')
    path_dl_cn.mkdir(parents=True, exist_ok=True)
    path_dl_pcn = Path(f'pcn_dl/QU_B_1/pcn')
    path_dl_pcn.mkdir(parents=True, exist_ok=True)
    path_dl_n = Path(f'pcn_dl/QU_B_1/n')
    path_dl_n.mkdir(parents=True, exist_ok=True)
    path_dl_removal = Path(f'pcn_dl/QU_B_1/removal_{threshold}sigma')
    path_dl_removal.mkdir(parents=True, exist_ok=True)
    path_dl_ps = Path(f'pcn_dl/QU_B_1/ps_{threshold}sigma')
    path_dl_ps.mkdir(parents=True, exist_ok=True)
    path_dl_inpaint_eb = Path(f'pcn_dl/QU_B_1/inpaint_eb_{threshold}sigma')
