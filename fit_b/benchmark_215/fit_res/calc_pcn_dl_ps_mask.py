...
def cpr_spectrum_pcn_b(bin_mask, apo_mask):
    ...
    m_ps_b = hp.read_map(f'../inpainting/input_b/{rlz_idx}.fits') * bin_mask
    ...
    path_dl_c = Path(f'pcn_dl/PS_MASK/c')
    path_dl_c.mkdir(parents=True, exist_ok=True)
    path_dl_cn = Path(f'pcn_dl/PS_MASK/cn')
    path_dl_cn.mkdir(parents=True, exist_ok=True)
    path_dl_pcn = Path(f'pcn_dl/PS_MASK/pcn')
    path_dl_pcn.mkdir(parents=True, exist_ok=True)
    path_dl_n = Path(f'pcn_dl/PS_MASK/n')
    path_dl_n.mkdir(parents=True, exist_ok=True)
    path_dl_removal = Path(f'pcn_dl/PS_MASK/removal_{threshold}sigma')
    path_dl_removal.mkdir(parents=True, exist_ok=True)
    path_dl_ps = Path(f'pcn_dl/PS_MASK/ps_{threshold}sigma')
    path_dl_ps.mkdir(parents=True, exist_ok=True)
    path_dl_inpaint_eb = Path(f'pcn_dl/PS_MASK/inpaint_eb_{threshold}sigma')
    path_dl_inpaint_eb.mkdir(parents=True, exist_ok=True)
    path_dl_inpaint_qu = Path(f'pcn_dl/PS_MASK/inpaint_qu_{threshold}sigma')
