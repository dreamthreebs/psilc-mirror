...
for rlz_idx in range(0,200):
    ...
    # n_ps_mask = np.load(f'./pcn_dl/PS_MASK/ps_3sigma/{rlz_idx}.npy')
    ...
    c = np.load(f'./pcn_dl/QU_B_1/c/{rlz_idx}.npy')
    # c_qu = np.load(f'./pcn_dl/QU/c/{rlz_idx}.npy')
    cn = np.load(f'./pcn_dl/QU_B_1/cn/{rlz_idx}.npy') - n_qu
...
plt.scatter(ell_arr, pcn_rmse, label='point source + CMB + noise ', marker='.')
plt.scatter(ell_arr, cn_rmse, label='CMB + noise', marker='.')
plt.scatter(ell_arr, rmv_qu_rmse, label='template fitting with fixed lon lat', marker='.')
plt.scatter(ell_arr, rmv_lon_lat_rmse, label='template fitting with free lon lat', marker='.')
...
plt.savefig('/afs/ihep.ac.cn/users/w/wangyiming25/tmp/20240507/215GHz.png', dpi=300)
plt.show()
