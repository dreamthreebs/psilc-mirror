...
class EBLeakageCorrection:
    def __init__(self, m, lmax, nside, mask, post_mask, method:str='cutqufitqu', check_res:bool=False, n_iter=3, slope=None):
    ...
    def zzr(self, lmax, nside):
        ...
        def linear_fitting():
            if self.slope is None:
                ...
                slope = self.slope
                print(f'slope is given and {slope=}')
    ...
    def fullqufitqu(self, lmax, nside):
        ...
        def linear_fitting():
            if self.slope is None:
                coeffs = np.polyfit(self.tmp_full_qu[1:2,self.mask>0].flatten(), self.crt_full_qu[1:2,self.mask>0].flatten(), 1)
                slope, intercept = coeffs
                print(f'{slope=}, {intercept=}')
            else:
                slope = self.slope
    ...
    def cutqufitqu(self, m, lmax, nside):
        ...
        def linear_fitting():
            if self.slope is None:
                coeffs = np.polyfit(self.tmp_cut_qu[1:2,self.mask>0].flatten(), self.crt_cut_qu[1:2,self.mask>0].flatten(), 1)
                slope, intercept = coeffs
                print(f'{slope=}, {intercept=}')
            else:
                slope = self.slope
