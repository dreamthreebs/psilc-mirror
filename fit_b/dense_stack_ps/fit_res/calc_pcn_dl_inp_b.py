...
def cpr_spectrum_pcn_b(bin_mask, apo_mask):
    ...
    path_dl_c = Path(f'pcn_dl/INP_B/c')
    path_dl_c.mkdir(parents=True, exist_ok=True)
    path_dl_cn = Path(f'pcn_dl/INP_B/cn')
    path_dl_cn.mkdir(parents=True, exist_ok=True)
    path_dl_pcn = Path(f'pcn_dl/INP_B/pcn')
    path_dl_pcn.mkdir(parents=True, exist_ok=True)
    path_dl_n = Path(f'pcn_dl/INP_B/n')
    path_dl_n.mkdir(parents=True, exist_ok=True)
    path_dl_removal = Path(f'pcn_dl/INP_B/removal_{threshold}sigma')
    path_dl_removal.mkdir(parents=True, exist_ok=True)
    path_dl_ps = Path(f'pcn_dl/INP_B/ps_{threshold}sigma')
    path_dl_ps.mkdir(parents=True, exist_ok=True)
    path_dl_inpaint_eb = Path(f'pcn_dl/INP_B/inpaint_eb_{threshold}sigma')
    path_dl_inpaint_eb.mkdir(parents=True, exist_ok=True)
    path_dl_inpaint_eb_n = Path(f'pcn_dl/INP_B/inpaint_eb_{threshold}sigma_n')
