...
class FitPolPS:
    ...
    def __init__(self, m_q, m_u, lon_rad, lat_rad, freq, nstd_q, nstd_u, flux_idx, df_mask, df_ps, lmax, nside, radius_factor, beam, sigma_threshold=5, epsilon=1e-4, debug_flag=False):
        ...
        logger.info(f'{self.qflux=}, {self.uflux=}')
        logger.info(f'q_amp={self.q_amp}, u_amp={self.u_amp}, p_amp={self.p_amp}')
    ...
    def fit_all(self, cov_mode:str, mode:str='pipeline'):
        ...
        def fit_1_ps():
            ...
            obj_minuit.limits = [(-50000,50000),(-50000,50000), (0,720), (-90,90), (-500,500), (-500,500)]
            logger.debug(f'\n{obj_minuit.migrad()}')
            logger.debug(f'\n{obj_minuit.hesse()}')
