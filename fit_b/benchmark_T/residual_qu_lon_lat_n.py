...
class GetResidual:
    ...
    def pcn_res(self, mask, threshold=2):
        ...
        m_pcn = self.gen_map(rlz_idx=rlz_idx)
        ...
        for flux_idx in range(10):
            ...
            pcn_p_amp = np.load(f'./fit_res/pcn_params/fit_qu_lon_lat_n/idx_{flux_idx}/fit_P_{rlz_idx}.npy')
            pcn_phi = np.load(f'./fit_res/pcn_params/fit_qu_lon_lat_n/idx_{flux_idx}/fit_phi_{rlz_idx}.npy')
            ...
            fit_lon = np.load(f'./fit_res/pcn_params/fit_qu_lon_lat/idx_{flux_idx}/fit_lon_{rlz_idx}.npy')
            fit_lat = np.load(f'./fit_res/pcn_params/fit_qu_lon_lat/idx_{flux_idx}/fit_lat_{rlz_idx}.npy')
            pcn_vec = np.asarray(hp.ang2vec(theta=fit_lon, phi=fit_lat, lonlat=True))
        ...
        path_for_res_map = Path(f'./fit_res/pcn_fit_lon_lat_n/{threshold}sigma')
...
def main():
    ...
    nside = 2048
    beam = 67
    df_mask = pd.read_csv(f'./mask/30.csv')
    radius_factor = 1.5
