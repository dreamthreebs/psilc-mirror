...
class GetResidual:
    ...
    def gen_map(self, rlz_idx):
        ...
        nstd = np.load(f'../../FGSim/NSTDNORTH/2048/{self.freq}.npy')
        ...
        cmb_iqu = hp.synfast(cls.T, nside=nside, fwhm=np.deg2rad(beam)/60, new=True, lmax=self.lmax)

        cls_fg = gen_fg_cl()
        np.random.seed(seed=fg_seed[rlz_idx])
        fg_iqu = hp.synfast(cls_fg, nside=nside, fwhm=0, new=True, lmax=self.lmax)
    ...
    def pcn_res(self, mask, threshold=2):
        ...
        m_pcn, _ = self.gen_map(rlz_idx=rlz_idx)
        ...
        for flux_idx in range(217):
            ...
            pcn_p_amp_true = np.load(f'./fit_res/pcfn_params/fit_qu_no_const/idx_{flux_idx}/P_{rlz_idx}.npy')
            pcn_phi_true = np.load(f'./fit_res/pcfn_params/fit_qu_no_const/idx_{flux_idx}/phi_{rlz_idx}.npy')
