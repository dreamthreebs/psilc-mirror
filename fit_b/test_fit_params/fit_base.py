...
class FitPolPS:
    ...
    def __init__(self, m_q, m_u, freq, nstd_q, nstd_u, flux_idx, df_mask, df_ps, lmax, nside, radius_factor, beam, sigma_threshold=5, epsilon=1e-4, debug_flag=False, cov_path='./cmb_qu_cov', cov_precise_path=None):
        ...
        self.lon = np.load('./data/ps/pix_lon1.npy') # latitude of the point sources in degree
        self.lat = np.load('./data/ps/pix_lat1.npy') # latitude of the point sources in degree
    ...
    def fit_all(self, cov_mode:str, mode:str='pipeline', return_qu:bool=False):
        ...
        def fit_1_ps():
            ...
            obj_minuit = Minuit(lsq_params, name=("q_amp_1","u_amp_1"), *params)
            obj_minuit.limits = [(-50000,50000),(-50000,50000)]
            logger.debug(f'\n{obj_minuit.migrad()}')
            logger.debug(f'\n{obj_minuit.hesse()}')
        ...
        def fit_2_ps():
            ...
            self.q_amp_2 = 4000
            self.u_amp_2 = -6000
            self.ctr2_lon = np.load('./data/ps/pix_lon2_3.npy')
            self.ctr2_lat = np.load('./data/ps/pix_lat2_3.npy')
            ...
            obj_minuit.limits = [(-50000,50000),(-50000,50000),(-50000,50000),(-50000,50000)]
            logger.debug(f'\n{obj_minuit.migrad()}')
            logger.debug(f'\n{obj_minuit.hesse()}')
...
def gen_pix_idx(flux_idx, freq, nside, beam):
    lmax = 500
    ...
    nstd_q = nstd[1].copy()
    nstd_u = nstd[2].copy()
    m = np.load('./data/map/pcfn_3_0.npy')
    m_q = m[1].copy()
    m_u = m[2].copy()
    ...
    df_mask = pd.read_csv(f'../{freq}GHz/mask/{freq}.csv')
    df_ps = df_mask
...
if __name__ == '__main__':
    gen_pix_idx(flux_idx=0, freq=30, nside=2048, beam=67)
    # gen_cov_inv()
