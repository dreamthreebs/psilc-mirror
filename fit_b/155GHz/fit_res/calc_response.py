...
def cpr_spectrum_pcn_b(bin_mask, apo_mask):
    ...
    m_pcfn_q = np.load(f'./std/4sigma/map_q_{rlz_idx}.npy') * bin_mask
    m_pcfn_u = np.load(f'./std/4sigma/map_u_{rlz_idx}.npy') * bin_mask

    m_n_q = np.load(f'./noise/4sigma/map_q_{rlz_idx}.npy') * bin_mask
    m_n_u = np.load(f'./noise/4sigma/map_u_{rlz_idx}.npy') * bin_mask
    ...
    path_dl_qu = Path(f'pcfn_dl4/RMV_4SIGMA/STD')
    path_dl_qu_n = Path(f'pcfn_dl4/RMV_4SIGMA/n')
