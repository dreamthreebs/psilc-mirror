...
def mean_and_std(sim_mode):
    for rlz_idx in range(1,200):
        ...
        n_ps_mask = np.load(f'./pcfn_dl4/RMV_4SIGMA/n/{rlz_idx}.npy')
        ps_mask = np.load(f'./pcfn_dl4/RMV_4SIGMA/STD/{rlz_idx}.npy') - n_ps_mask

        # n_ps_mask = np.load(f'./pcfn_dl4/MASK/noise/{rlz_idx}.npy')
        # ps_mask = np.load(f'./pcfn_dl4/MASK/STD/{rlz_idx}.npy') - n_ps_mask

        n_inp = np.load(f'./pcfn_dl4/RMV_5SIGMA/n/{rlz_idx}.npy')
        inp = np.load(f'./pcfn_dl4/RMV_5SIGMA/STD/{rlz_idx}.npy') - n_inp
...
ax1.errorbar(ell_arr * 1.015, rmv_mean[:lmax_ell_arr], yerr=rmv_std[:lmax_ell_arr], label='GPSF 3sigma', fmt='.', color='green', capsize=s)
ax1.errorbar(ell_arr * 1.03, ps_mask_mean[:lmax_ell_arr], yerr=ps_mask_std[:lmax_ell_arr], label='GPSF 4sigma', fmt='.', color='orange', capsize=s)
ax1.errorbar(ell_arr * 1.045, inp_mean[:lmax_ell_arr], yerr=inp_std[:lmax_ell_arr], label='GPSF 5sigma', fmt='.', color='red', capsize=s)
...
path_fig = Path('/afs/ihep.ac.cn/users/w/wangyiming25/tmp/20251028')
