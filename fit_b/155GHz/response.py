...
def get_ps_need_process(n_ps, threshold=3.0):
    ...
    filtered_df.to_csv(f'./mask/{freq}_after_filter_{threshold}sigma.csv', index=True)
...
class GetResidual:
    ...
    def pcn_res(self, mask, threshold=2):
        # use df_rlz in model reconstruction!!!
        ...
        for flux_idx in range(len(self.df_mask)):
            ...
            pcn_fit_lon = np.rad2deg(df_rlz.at[flux_idx, 'lon'])
            pcn_fit_lat = np.rad2deg(df_rlz.at[flux_idx, 'lat'])
            ...
            P = (pcn_q_amp + 1j * pcn_u_amp) * self.nside2pixarea_factor * np.exp(2j * df_rlz.at[flux_idx, 'lon'])
            lugwid_P = P * profile
...
def main():
    df_mask = pd.read_csv(f'./mask/{freq}_after_filter_5sigma.csv')
    radius_factor = 1.5
    ...
    obj.pcn_res(mask, threshold=5)


if __name__=="__main__":
    # get_ps_need_process(n_ps=ps_number, threshold=5)
    main()
