...
obj = EBLeakageCorrection(m=np.asarray([t,q,u]), lmax=3*nside-1, nside=nside, mask=mask, post_mask=mask)
_,_,cln_b = obj.run_eblc()
slope_fg = obj.return_slope()

# path_b = Path('./3sigma/B_fg')
# path_b.mkdir(exist_ok=True, parents=True)
# np.save(path_b / Path(f'{rlz_idx}.npy'), cln_b)

path_slope = Path(f'./eblc_slope_pcfn')
path_slope.mkdir(exist_ok=True, parents=True)

np.save(path_slope / Path(f'{rlz_idx}.npy'), slope_fg)
