...
def test_fg():
    ...
    cl_TE = np.zeros_like(cl_BB)

    cls_fg = np.array([cl_TT, cl_EE, cl_BB, cl_TE])
    ...
    # cmb_rlz = hp.synfast(cls_cmb, nside=nside, new=True, fwhm=np.deg2rad(beam)/60)

    # cf_rlz = hp.anafast(fg_rlz + cmb_rlz, lmax=lmax)
    ...
    plt.plot(l*(l+1)*cls_fg[2,:lmax+1]/(2*np.pi), label='fg input B')
    plt.plot(l*(l+1)*cls_cmb[2,:lmax+1]/(2*np.pi), label='cmb input B')
    plt.plot(l*(l+1)*cls_fg[1,:lmax+1]/(2*np.pi), label='fg input E')
    plt.plot(l*(l+1)*cls_cmb[1,:lmax+1]/(2*np.pi), label='cmb input E')
    plt.plot(l*(l+1)*cls_fg[2,:lmax+1]/bl**2/(2*np.pi), label=f'fg debeam')
    plt.semilogy()
    plt.ylim(1e-5,1e5)
    plt.xlabel('$\\ell$')
    plt.ylabel('$D_\\ell$')
