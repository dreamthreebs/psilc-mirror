...
def test_cmb():
    ...
    cmb_rlz = hp.synfast(cls_cmb[0], nside=nside)
    ...
    # plt.plot(l*(l+1)*cls_rlz/(2*np.pi), label='one realization')
    # plt.legend()
    # plt.show()
...
def check_cmb():
    ...
    cls_list = []
    for i in range(1000):
        cls_rlz = np.load(f'./data/cl_rlz/{i}.npy')
    ...
    plt.figure(2)
    plt.plot((cls_cmb[0,:lmax+1] - cls_mean)/cls_cmb[0,:lmax+1])
...
def test_mode_mix():
    ...
    m = get_cmb()
    dl = calc_dl_from_scalar_map(scalar_map=m*bin_mask, apo_mask=apo_mask, bin_dl=bin_dl, masked_on_input=False)
    cl_no_mask = hp.anafast(m, lmax=lmax)

    cls_cmb = np.load('../../src/cmbsim/cmbdata/cmbcl_8k.npy').T
    dl_true = bin_dl.bin_cell(cls_cmb[0,:lmax+1])
    dl_no_mask = bin_dl.bin_cell(cl_no_mask)
    print(f'{dl_true=}')

    path_dl_rlz = Path('./data/dls_cmb_lmax600')
...
def check_mode_mix():
    ...
    dl_partial_list = []
    for i in range(1000):
        dl_full = np.load(f'./data/dls_cmb_lmax600/full_{i}.npy')
        dl_partial = np.load(f'./data/dls_cmb_lmax600/partial_{i}.npy')
        print(f'{dl_full=}')
        print(f'{dl_partial=}')
