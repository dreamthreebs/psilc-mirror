...
class FitPointSource:
    def __init__(self, m, nstd, nside, flux_idx, radius_factor, beam, distance_factor, sigma_threshold=5, epsilon=1e-4, debug_flag=False):
        ...
        self.lon = np.load(f'./data/ps/pix_lon2_{distance_factor}.npy')
        self.lat = np.load(f'./data/ps/pix_lat2_{distance_factor}.npy')
    ...
    def fit_all(self, cov_mode:str, mode:str='pipeline'):
        ...
        def fit_3_params():
            ...
            self.ctr2_iflux = 481
            self.ctr2_lon = np.load(f'./data/ps/pix_lon1.npy')
            self.ctr2_lat = np.load(f'./data/ps/pix_lat1.npy')
            
            params = (1e1, self.ctr2_iflux, 0)
            ...
            obj_minuit.fixed["norm_beam2"] = True

            obj_minuit.limits = [(-1000,1000),(-1000,1000), (-1000,1000)]
            logger.debug(f'\n{obj_minuit.migrad()}')
            logger.debug(f'\n{obj_minuit.hesse()}')
...
def main():
    ...
    distance_factor = 0.5
    m = gen_ps(beam=beam, nside=nside, distance_factor=distance_factor, rlz_idx=rlz_idx, flux_t1=0)
    ...
    _,_,fit_norm,_,_ = obj.fit_all(cov_mode='noise')
    print(f'{fit_norm=}')
