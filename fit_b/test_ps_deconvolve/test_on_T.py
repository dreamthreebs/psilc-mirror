...
def gen_ps():
    ...
    m = np.zeros(npix)
    m[ipix] = delta_I
    smooth_m = hp.smoothing(m, fwhm=np.deg2rad(beam)/60)

    np.save(f'./ps_{nside}.npy', smooth_m)
...
def gen_ps_beam_out():
    ...
    m = np.zeros(npix)
    m[ipix] = delta_I
    smooth_m = hp.smoothing(m, fwhm=np.deg2rad(beam_out)/60)

    np.save(f'./ps_{nside}_{beam_out}.npy', smooth_m)
...
def deconvolve():
    ...
    np.save(f'./ps_{nside}_{beam}to{beam_out}.npy', deconvolve_m)
...
def deconvolve_ps_spectrum():
    m = np.load(f'./ps_{nside}.npy')
    m1 = np.load(f'./ps_{nside}_{beam}to{beam_out}.npy')
    m2 = np.load(f'./ps_1024_11.npy')
