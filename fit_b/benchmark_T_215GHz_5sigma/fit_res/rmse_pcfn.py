...
for rlz_idx in range(0,200):
    ...
    pcfn = np.load(f'./pcfn_dl/RMV/pcfn/{rlz_idx}.npy')
    ...
    inp_eb = np.load(f'./pcfn_dl/RMV/inp_m2/{rlz_idx}.npy')
    ...
    plt.plot(ell_arr, pcfn, label=f'pcfn {rlz_idx}')
    plt.plot(ell_arr, n_inp, label=f'n inp {rlz_idx}')
    plt.plot(ell_arr, n_rmv, label=f'n rmv {rlz_idx}')
...
pcfn_rmse = np.sqrt(np.sum((pcfn_arr-cf_arr) ** 2, axis=0) / nsim)[:18]
print(f'{pcfn_rmse.shape=}')
# cfn_rmse = np.sqrt(np.sum((cfn_arr-cf_arr) ** 2, axis=0) / nsim)[:18]
rmv_qu_rmse = np.sqrt(np.sum((rmv_qu_arr-cf_arr) ** 2, axis=0) / nsim)[:18]
# rmv_lon_lat_rmse = np.sqrt(np.sum((rmv_lon_lat_arr-c_arr) ** 2, axis=0) / nsim)
inp_eb_rmse = np.sqrt(np.sum((inp_eb_arr-cf_arr) ** 2, axis=0) / nsim)[:18]
# ps_mask_rmse = np.sqrt(np.sum((ps_mask_arr-cf_arr) ** 2, axis=0) / nsim)
qu_mask_rmse = np.sqrt(np.sum((qu_mask_arr-cf_arr) ** 2, axis=0) / nsim)[:18]

cf_mean = np.mean(cf_arr, axis=0)[:18]
print(f'{ell_arr[1:7]=}')
...
path_save = Path('/afs/ihep.ac.cn/users/w/wangyiming25/tmp/20241111')
path_save.mkdir(exist_ok=True, parents=True)
plt.savefig(path_save / Path('215GHz_184ps_rmse.png'), dpi=300)
...
path_save = Path('/afs/ihep.ac.cn/users/w/wangyiming25/tmp/20241111')
path_save.mkdir(exist_ok=True, parents=True)
plt.savefig(path_save / Path('cpr_results_215GHz_184ps.png'), dpi=300)
