...
class GnomProj:
    ...
    def fit_ps_ns_plane(self, beam):
        ...
        n_pts = self.xsize * self.ysize
        def lsq(reso_bias, norm_beam, const):
            lon_fit = self.lon
            lat_fit = self.lat
            reso_fit = self.reso + reso_bias
            self.gproj_obj = hp.projector.GnomonicProj(rot=[lon_fit, lat_fit, 0], xsize=self.xsize, ysize=self.ysize, reso=reso_fit)
        ...
        obj_minuit = Minuit(lsq, reso_bias=5, norm_beam=1,  const=0)
        obj_minuit.limits = [(-1.7,10), (0,10),(-1e2,1e2)]
...
if __name__ == '__main__':
    ...
    xsize = 40
    ysize = 40
    reso = 1 * hp.nside2resol(nside=2048, arcmin=True)
