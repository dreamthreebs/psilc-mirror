...
def calculate_cov(params):
    i, j, ipix_fit, nside, lmax, cl, loaded_itp_funcs = params
    ipix_i = ipix_fit[i]
    ipix_j = ipix_fit[j]
...
def main():
    ...
    cov = np.zeros((n_cov, n_cov))  # Assuming cov is a square matrix of size n_cov

    with ProcessPoolExecutor() as executor:
        # Using a chunksize to improve performance on large numbers of tasks
        chunksize = max(1, len(tasks) // (executor._max_workers * 4))
        futures = list(tqdm(executor.map(calculate_cov, tasks, chunksize=chunksize), total=len(tasks)))

    # Populate the covariance matrix
    for i, j, cov_ij in futures:
        cov[i, j] = cov_ij
        if i != j:
            cov[j, i] = cov_ij
