...
ipix_fit = hp.query_disc(nside=nside, vec=center_vec, radius=3 * np.deg2rad(beam)/60)
...
def fit_model(theta, norm_beam, sigma, norm_beam2, sigma2, const):
    beam_profile = norm_beam / (2*np.pi*sigma**2) * np.exp(- (theta)**2 / (2 * sigma**2)) + norm_beam2 / (2*np.pi*sigma2**2) * np.exp(- (theta)**2 / (2 * sigma2**2))
...
obj_minuit = Minuit(lsq, norm_beam=1, sigma=0.0070, norm_beam2=3, sigma2=0.0088, const=0)
obj_minuit.limits = [(0,10),(1e-6,1e-2),(0,10),(1e-6,1e-2),(-100,100)]
