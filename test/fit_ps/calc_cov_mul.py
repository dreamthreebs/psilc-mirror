...
def calculate_cov(params):
    i, j, ipix_fit, nside, lmax, cl, loaded_itp_funcs, local_cache = params
    ipix_i = ipix_fit[i]
    ipix_j = ipix_fit[j]
    ...
    if cos_theta not in local_cache:
        cov_ij = calc_C_theta_itp(x=cos_theta, lmax=lmax, cl=cl[0:lmax+1], itp_funcs=loaded_itp_funcs)
        local_cache[cos_theta] = cov_ij
    else:
        cov_ij = local_cache[cos_theta]
...
def main():
    ...
    with ProcessPoolExecutor() as executor:
        chunksize = max(1, len(tasks) // (executor._max_workers * 10))
