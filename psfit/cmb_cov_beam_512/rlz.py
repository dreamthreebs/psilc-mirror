...
if __name__ == '__main__':
    ...
    # m = np.load('../../src/cmbsim/cmbdata/rlz_nside512/0.npy')
    # obj = FitPointSource(m=m, flux_idx=flux_idx, df_mask=df_mask, df_ps=df_ps, lon=lon, lat=lat, lmax=lmax, nside=nside, radius_factor=0.6, beam=beam)
    # obj.see_true_map(m, lon, lat, nside, beam)
    
    data_list = []
    for i in range(100):
        print(f"{i=}")
        sm = np.load(f'../../src/cmbsim/cmbdata/rlz_nside512/40GHz/{i}.npy')
        print(f"{np.mean(sm)=}")
        ...
        obj = FitPointSource(m=sm, flux_idx=flux_idx, df_mask=df_mask, df_ps=df_ps, lon=lon, lat=lat, lmax=lmax, nside=nside, radius_factor=0.6, beam=beam)

        # obj.see_true_map(m=sm, lon=lon, lat=lat, nside=nside, beam=beam)
        ipix_fit = obj.return_ipix_fit()
        my_map = sm[ipix_fit]
        print(f'{my_map.shape=}')
        # np.save(f'./datarlz/{i}.npy', my_map)
        data_list.append(my_map)
