...
class FitPointSource:
    ...
    def __init__(self, m, nstd, flux_idx, df_mask, df_ps, cl_cmb, lon, lat, iflux, lmax, nside, radius_factor, beam, sigma_threshold=5):
        ...
        ctr_vec = np.array(hp.ang2vec(theta=self.lon, phi=self.lat, lonlat=True))
        self.ipix_fit = hp.query_disc(nside=self.nside, vec=ctr_vec, radius=self.radius_factor * np.deg2rad(self.beam) / 60)
    ...
    def calc_covariance_matrix(self, mode='cmb+noise'):
        cmb_cov_path = f'./cov/{self.flux_idx}.npy'
    ...
    def fit_ps(self, mode:str='pipeline', ps_pos_bias=0.05):
        ...
        if self.do_2_params == True:
            ...
            if chi2dof < 1.044:
                ...
                print(f'{fit_error=}')
                return obj_minuit.errors['norm_beam'], self.fit_norm_beam, fit_lon, fit_lat
            else:
                print('error flag here!')
                self.do_3_params = True
        ...
        if self.do_3_params == True:
            ...
            if chi2dof < 1.065:
                ...
                fit_error = np.abs((self.fit_norm_beam - self.ini_norm_beam) / self.ini_norm_beam)
                print(f'{fit_error=}')
                return obj_minuit.errors['norm_beam1'], self.fit_norm_beam, fit_lon, fit_lat
            else:
                print('error flag here!')
                self.do_4_params = True
