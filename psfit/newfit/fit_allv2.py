...
class FitPointSource:
    def __init__(self, m, nstd, flux_idx, df_mask, df_ps, cl_cmb, lon, lat, iflux, lmax, nside, radius_factor, beam, lmin=171, sigma_threshold=5):
...
if __name__ == '__main__':
    ...
    for lmin in np.linspace(200,220, 21):
        ...
        obj = FitPointSource(m=m, nstd=nstd, flux_idx=flux_idx, df_mask=df_mask, df_ps=df_ps, cl_cmb=cl_cmb, lon=lon, lat=lat, iflux=iflux, lmax=lmax, nside=nside, radius_factor=1.0, beam=beam, lmin=lmin)
    ...
    # obj.see_true_map(m=m, lon=lon, lat=lat, nside=nside, beam=beam)
        ...
        if not os.path.exists(f'./covlmin/{lmin}'):
            os.makedirs(f'./covlmin/{lmin}')
        obj.calc_C_theta(save_path=f'./covlmin/{lmin}')

    # # obj.calc_covariance_matrix(mode='cmb', cmb_cov_fold='./cov')
    # obj.calc_covariance_matrix(mode='cmb+noise', cmb_cov_fold='./cov')
    # obj.fit_all()
