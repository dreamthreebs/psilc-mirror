...
def main():
    m = np.load('../../FGSim/FITDATA/256/PSCMB/40.npy')[0]
    ...
    if not os.path.exists('./PSCMB256'):
        os.mkdir('./PSCMB256')
    np.save("./PSCMB256/num_ps.npy", np.array(num_ps_list))
    np.save("./PSCMB256/chi2dof.npy", np.array(chi2dof_list))
    np.save("./PSCMB256/norm_beam.npy",np.array(norm_beam_list))
    np.save("./PSCMB256/norm_error.npy",np.array(norm_error_list))
    np.save("./PSCMB256/fit_lon.npy",np.array(fit_lon_list))
    np.save("./PSCMB256/fit_lat.npy",np.array(fit_lat_list))
