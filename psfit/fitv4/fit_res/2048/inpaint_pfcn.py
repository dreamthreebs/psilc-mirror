...
def gen_mask(ori_mask, nside, radius_factor, beam):
    for rlz_idx in range(100):
        print(f'{rlz_idx=}')
        mask_list = np.load(f'./ps_cmb_fg_noise_residual/mask{rlz_idx}.npy')
        print(f'{mask_list=}')

        # m = np.load('./ps_cmb_noise_residual/map0.npy')
        # m = np.load('../../../../fitdata/synthesis_data/2048/PSCMBFGNOISE/40/0.npy')[0]
        ...
        for flux_idx in mask_list:
            print(f'{flux_idx=}')
            pcn_fit_lon = np.load(f'./PSCMBFGNOISE/1.5/idx_{flux_idx}/fit_lon.npy')[rlz_idx]
            pcn_fit_lat = np.load(f'./PSCMBFGNOISE/1.5/idx_{flux_idx}/fit_lat.npy')[rlz_idx]
        ...
        hp.write_map(f'./for_inpainting/mask/pfcn/{rlz_idx}.fits', mask, overwrite=True)
