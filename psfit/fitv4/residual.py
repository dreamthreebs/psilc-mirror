...
class GetResidual:
    ...
    def psnoise(self, mask):
        ...
        for idx_rlz in range(100):
            ...
            for flux_idx in range(136):
                ...
                # pcn_fit_lon = np.load(f'./fit_res/2048/PSNOISE/1.5/idx_{flux_idx}/fit_lon.npy')
                # pcn_fit_lat = np.load(f'./fit_res/2048/PSNOISE/1.5/idx_{flux_idx}/fit_lat.npy')
            ...
            hp.orthview(m_ps_noise*mask, rot=[100,50,0], half_sky=True, title='ps_noise map')
            hp.orthview(de_ps_map*mask, rot=[100,50,0], half_sky=True, title='de_ps map')
            hp.orthview(res_map*mask, rot=[100,50,0], half_sky=True, title='residual map:de_ps - noise map')
            plt.show()
    ...
    def ps_cmbnoise(self, mask):
        for idx_rlz in range(100):
            ...
            # hp.orthview(de_ps_map*mask, rot=[100,50,0], half_sky=True, title='de_ps map')
            # hp.orthview(res_map*mask, rot=[100,50,0], half_sky=True, title='residual map:de_ps - cmb noise map')
            # plt.show()

            path_for_res_map = Path('./fit_res/2048/ps_cmb_noise_residual')
    ...
    def ps_fg_cmbnoise(self, mask):
        for idx_rlz in range(100):
            ...
            m_ps_cmb_noise = np.load(f'../../fitdata/synthesis_data/2048/PSCMBFGNOISE/40/{idx_rlz}.npy')[0].copy()
            m_no_ps = np.load(f'../../fitdata/synthesis_data/2048/CMBFGNOISE/40/{idx_rlz}.npy')[0].copy()
            de_ps_map = m_ps_cmb_noise.copy()

            for flux_idx in range(116, 117):
            ...
            path_for_res_map = Path('./fit_res/2048/ps_cmb_fg_noise_residual')
            path_for_res_map.mkdir(parents=True, exist_ok=True)
            # np.save(path_for_res_map / Path(f'{idx_rlz}.npy'), res_map)
