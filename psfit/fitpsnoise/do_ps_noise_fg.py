...
def main1():
    m = np.load('../../FGSim/FITDATA/PSFGNOISE/40.npy')[0]
    nstd = np.load('../../FGSim/NSTDNORTH/2048/40.npy')[0]
    df_mask = pd.read_csv('../partial_sky_ps/ps_in_mask/mask40.csv')
    df_ps = pd.read_csv('../../test/ps_sort/sort_by_iflux/40.csv')
    ...
    np.save("./data/PSFGNOISE/norm_beam.npy",np.array(norm_beam_list))
    np.save("./data/PSFGNOISE/fit_lon.npy",np.array(fit_lon_list))
    np.save("./data/PSFGNOISE/fit_lat.npy",np.array(fit_lat_list))
