    ...
    def fit_ps_ns(self, mode:str='pipeline', ps_pos_bias=0.05):
        ...
        def lsq_ps(*args):
            ...
            for i in range(num_ps):
                norm_beam = args[i]
                ctr_vec = np.array(hp.pix2vec(nside=self.nside, ipix=ctr_ipix_disc[i]))
        ...
        if chi2dof < 1.04:
            if is_ps:
                ...
                print(f'{fit_error=}')
                if fit_error > 0.1:
                    raise ValueError('fit_error > 0.1, please check')
                else:
        ...
        if self.do_3_params == True:
            ...
            print(f'{chi2dof_arr=}')
            min_chi2dof = np.min(chi2dof_arr)
            min_chi2dof_idx = np.unravel_index(np.argmin(chi2dof_arr), chi2dof_arr.shape)
        ...
        if self.do_4_params == True:
            ...
            chi2dof_arr = np.zeros((len(ctr1_ipix_disc), len(ctr2_ipix_disc), len(ctr3_ipix_disc)))
            print(f'{chi2dof_arr.shape}')

            for idx_1, ipix_1 in enumerate(ctr1_ipix_disc):
                for idx_2, ipix_2 in enumerate(ctr2_ipix_disc):
                    for idx_3, ipix_3 in enumerate((ctr3_ipix_disc)):
                        ctr_ipix_disc = (ipix_1, ipix_2, ipix_3)
