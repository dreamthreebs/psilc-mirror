...
from fitcmbfg2 import FitPointSource

def main():
    m = np.load('../../FGSim/FITDATA/PSNOISE/2048/40.npy')[0]
...
def main1():
    m = np.load('../../FGSim/FITDATA/PSCMBFG/40.npy')[0]
    nstd = np.load('../../FGSim/NSTDNORTH/2048/40.npy')[0]
    df_mask = pd.read_csv('../partial_sky_ps/ps_in_mask/mask40.csv')
    df_ps = pd.read_csv('../../test/ps_sort/sort_by_iflux/40.csv')
    ...
    for flux_idx in range(0,137):
        print(f'{flux_idx=}')
        lon = np.rad2deg(df_mask.at[flux_idx, 'lon'])
        lat = np.rad2deg(df_mask.at[flux_idx, 'lat'])
        iflux = df_mask.at[flux_idx, 'iflux']

        obj = FitPointSource(m=m, nstd=nstd, flux_idx=flux_idx, df_mask=df_mask, df_ps=df_ps, cl_cmb=cl_cmb, lon=lon, lat=lat, iflux=iflux, lmax=lmax, nside=nside, radius_factor=1.0, beam=beam, sigma_threshold=3.0)
        ...
        norm_error, norm_beam, fit_lon, fit_lat =  obj.fit_ps()

        print(f'{norm_error=}, {norm_beam=}, {fit_lon=}, {fit_lat=}')
        norm_beam_list.append(norm_beam)
        fit_lon_list.append(fit_lon)
        fit_lat_list.append(fit_lat)
        norm_error_list.append(norm_error)
    np.save("./fit_data2/PSCMBFG/norm_beam.npy",np.array(norm_beam_list))
    np.save("./fit_data2/PSCMBFG/fit_lon.npy",np.array(fit_lon_list))
    np.save("./fit_data2/PSCMBFG/fit_lat.npy",np.array(fit_lat_list))
    np.save("./fit_data2/PSCMBFG/norm_error.npy",np.array(norm_error_list))
