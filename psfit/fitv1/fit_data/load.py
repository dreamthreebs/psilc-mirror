...
norm_beam = np.load('./3sigma/norm_beam.npy')
lon = np.load('./3sigma/fit_lon.npy')
lat = np.load('./3sigma/fit_lat.npy')
...
df["true_norm"] = norm_beam_true
df["fit_error"] = fit_error
df["fit_lon"] = lon_all
df["fit_lat"] = lat_all

df.to_csv(f'./3sigma/40.csv', index=False)
