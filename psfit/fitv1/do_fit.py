...
from fitcmbfg1 import FitPointSource
...
def main1():
    ...
    for flux_idx in range(100,137):
        ...
        obj.calc_covariance_matrix()
        norm_beam, fit_lon, fit_lat =  obj.fit_ps()

        print(f'{norm_beam=}, {fit_lon=}, {fit_lat=}')
        norm_beam_list.append(norm_beam)
        fit_lon_list.append(fit_lon)
        fit_lat_list.append(fit_lat)
    np.save("./fit_data1/3sigma/norm_beam3.npy",np.array(norm_beam_list))
    np.save("./fit_data1/3sigma/fit_lon3.npy",np.array(fit_lon_list))
    np.save("./fit_data1/3sigma/fit_lat3.npy",np.array(fit_lat_list))
