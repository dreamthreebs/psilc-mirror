...
from fit_ps_ns import FitPointSource
...
def main1():
    ...
    for flux_idx in range(0,139):
        ...
        # obj.find_first_second_nearby_ps_lon_lat()
        # obj.fit_ps_ns(mode='10params')
        num_ps, chi2dof, norm_beam, norm_error, fit_lon, fit_lat =  obj.fit_ps_ns()
    ...
    np.save("./ps_ns_data/num_ps.npy", np.array(num_ps_list))
    np.save("./ps_ns_data/chi2dof1.npy", np.array(chi2dof_list))
    np.save("./ps_ns_data/norm_beam1.npy",np.array(norm_beam_list))
    np.save("./ps_ns_data/norm_error1.npy",np.array(norm_error_list))
    np.save("./ps_ns_data/fit_lon1.npy",np.array(fit_lon_list))
    np.save("./ps_ns_data/fit_lat1.npy",np.array(fit_lat_list))
