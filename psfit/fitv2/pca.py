...
class FitPointSource:
    ...
    def invert_matrix_using_cholesky(self, matrix):
    ...
    def calc_covariance_matrix(self, mode='cmb+noise', cmb_cov_fold='../fit/cov'):
        ...
        if mode == 'cmb':
            ...
            # self.inv_cov = self.invert_matrix_using_cholesky(cov)
            self.cov = cov
    ...
    def fit_all(self, mode:str='pipeline'):
        ...
        principal_components = pca.fit_transform(self.cov)
        print(f'{principal_components.shape=}')

        data_pca = np.dot(data, principal_components)
        model_pca = np.dot(model, principal_components)

        cov_pca = np.dot(principal_components.T, np.dot(self.cov, principal_components))
        print(f'{cov_pca=}')

        inv_cov = np.linalg.inv(cov_pca)
        # inv_cov = np.linalg.solve(cov_pca, )

        def lsq_1_params(const):

            y_diff = data_pca - model_pca
...
if __name__ == '__main__':
    ...
    m = np.load('../../inpaintingdata/CMBREALIZATION/40GHz/3.npy')[0]
    ...
    obj.calc_covariance_matrix(mode='cmb', cmb_cov_fold='../cmb_cov_precise_calc/cov4')
    obj.fit_all()
