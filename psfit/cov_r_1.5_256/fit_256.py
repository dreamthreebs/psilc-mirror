...
class FitPointSource:
    ...
    def calc_precise_C_theta(self, save_path='./precise_cov'):
        from numpy.polynomial.legendre import Legendre

        def calc_Pl_x(lmax, x):
            Plx = np.zeros(lmax+1)
            for l in range(lmax+1):
                Plx[l] = Legendre.basis(l)(x)
            return Plx
            ...
            for j in range(i+1):
                if i == j:
                    ...
                    if cos_theta not in theta_cache:
                        cov_ij = 1 / (4 * np.pi) * np.sum((2 * np.arange(self.lmax + 1) + 1) * self.cl_cmb[:self.lmax+1] * calc_Pl_x(self.lmax, cos_theta))
    ...
    def calc_C_theta(self, save_path='./cov'):
        ...
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        np.save(os.path.join(save_path,f'{self.flux_idx}.npy'), cov)
        memory_size = sys.getsizeof(theta_cache)
        print(f'{memory_size=}')
...
if __name__ == '__main__':
    m = np.load('../../fitdata/synthesis_data/256/PSNOISE/40/0.npy')[0]
    ...
    obj = FitPointSource(m=m, nstd=nstd, flux_idx=flux_idx, df_mask=df_mask, df_ps=df_ps, cl_cmb=cl_cmb, lon=lon, lat=lat, iflux=iflux, lmax=lmax, nside=nside, radius_factor=3.0, beam=beam, epsilon=1e-5)
    ...
    obj.calc_C_theta(save_path='./cov_1')
