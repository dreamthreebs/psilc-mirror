...
def calc_avg_var():
    ...
    for rlz_idx in range(100):
        print(f'{rlz_idx=}')
        m_rlz = np.load(f'../ps_cmb_fg_noise_residual/2sigma/map{rlz_idx}.npy')
        m_list.append(m_rlz)
    
    m_avg = np.sum(np.asarray(m_list), axis=0) / 100
    m_var = np.var(np.asarray(m_list), axis=0)

    np.save('./pcfn/2sigma/ps_res_avg.npy', m_avg)
    np.save('./pcfn/2sigma/ps_res_var.npy', m_var)

calc_avg_var()

#m_avg = np.load('./pcfn/2sigma/ps_res_avg.npy')
#m_var = np.load('./pcfn/2sigma/ps_res_var.npy')
#m_one_rlz = np.load(f'../ps_cmb_fg_noise_residual/2sigma/map3.npy')
#
#flux_idx = 5
#
