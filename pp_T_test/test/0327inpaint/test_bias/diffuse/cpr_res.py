...
def see_avg_var():
    ...
    inp_mean = np.load('./avg_3000/inp_mean.npy')
    ...
    inp_mean_isap = np.load('../avg_var/inp_mean.npy')
    ...
    plt.plot(inp_mean/bl**2, label='diffuse inpaint')
    plt.plot(inp_mean_isap/bl**2, label='iSAP inpaint')
    ...
    # plt.plot(np.abs(np.sqrt(cn_var)/cn_mean), label='cv ratio')
