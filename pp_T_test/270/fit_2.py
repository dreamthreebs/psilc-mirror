...
class FitPointSource:
    ...
    def check_all_def_fixed_cmb_cov(self):
        import glob
        file_list = glob.glob('./semi_def_cmb_cov_2048/r_1.5/*.npy')
        for idx, file in enumerate(file_list):
            logger.debug(f'{file=}')
            cov = np.load(file)
            cov  = cov + 0.05 * np.eye(cov.shape[0])
            inv_cov = np.linalg.solve(cov, np.eye(cov.shape[0]))
            I = cov @ inv_cov
            logger.debug(f'{I=}')
            is_correct = np.allclose(I, np.eye(cov.shape[0]))
...
def main():
    ...
    m = np.load(f'../../fitdata/2048/CMB/{freq}/0.npy')[0]
    ...
    # obj.check_all_def_fixed_cmb_cov()

    obj.fit_all(cov_mode='cmb')
