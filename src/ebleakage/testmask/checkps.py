...
cl_corrupted = hp.anafast(corrupted, lmax=lmax)
cl_corrected = hp.anafast(corrected, lmax=lmax)
plt.loglog(l*(l+1)*cl_corrupted/(2*np.pi)/fsky1, label=f'C1 5 corrupted', color='purple')
plt.loglog(l*(l+1)*cl_corrected/(2*np.pi)/fsky1, label=f'C1 5 corrected QU', color='purple', linestyle='--')
...
plt.loglog(l*(l+1)*cl_corrected/(2*np.pi)/fsky1, label=f'C1 5 corrected B', color='purple', linestyle=':')

# corrected = np.load(f'./20/cleaned_B.npy')
# corrupted = np.load(f'./21/corrupted_B.npy')
# cl_corrupted = hp.anafast(corrupted, lmax=lmax)
# cl_corrected = hp.anafast(corrected, lmax=lmax)
# plt.loglog(l*(l+1)*cl_corrupted/(2*np.pi)/fsky8, label=f'C1 5 corrupted QU nside 512')
# plt.loglog(l*(l+1)*cl_corrected/(2*np.pi)/fsky8, label=f'C1 5 corrected QU nside 512')
...
# plt.loglog(l*(l+1)*cl_corrected/(2*np.pi)/fsky8, label=f'C1 5 corrected B nside 512')
