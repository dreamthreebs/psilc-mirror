...
pars = camb.read_ini(os.path.join(camb_path,'inifiles','forebleakage.ini'))
...
for index, flag in enumerate(['TT', 'EE', 'BB']):
    plt.loglog(l*(l*1)*unlensedcl[:,index]/(2*np.pi),label=f'{flag} lensed')
...
np.save('./cmbdata/cmbcl.npy', unlensedcl)
