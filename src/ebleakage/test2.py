...
class EBCorrection:
    ...
    def fit_QU(self, lmax, nside):
        coeffs = np.polyfit(self.QU_B2[1:2].flatten(), self.QU_B1[1:2].flatten(), 1)
        slope, intercept = coeffs
        print(f"Slope: {slope}, Intercept: {intercept}")
        cleaned_QU = self.QU_B1 - slope * self.QU_B2
        self.cleaned_QU = self.add_post_mask(cleaned_QU)
        self.corrupted_QU = self.add_post_mask(self.QU_B1)
        self.template_QU = self.add_post_mask(slope * self.QU_B2)
    def fit_QU_B(self, lmax, nside):
        coeffs = np.polyfit(self.QU_B2[1:2].flatten(), self.QU_B1[1:2].flatten(), 1)
        slope, intercept = coeffs
        print(f"Slope: {slope}, Intercept: {intercept}")
        cleaned_QU = self.QU_B1 - slope * self.QU_B2
        self.cleaned_B = self.add_post_mask(hp.alm2map(hp.map2alm(cleaned_QU, lmax=lmax)[2], nside=nside))
        self.corrupted_B = self.add_post_mask(hp.alm2map(hp.map2alm(self.QU_B1, lmax=lmax)[2], nside=nside))
        self.template_B = self.add_post_mask(hp.alm2map(hp.map2alm(slope * self.QU_B2, lmax=lmax)[2], nside=nside))

    def fit_B(self, lmax, nside):
        coeffs = np.polyfit(self.template_B, self.corrupted_B, 1)
        slope, intercept = coeffs
        print(f"Slope: {slope}, Intercept: {intercept}")
        self.cleaned_B = self.add_post_mask(self.corrupted_B - slope * self.template_B)
        self.corrupted_B = self.add_post_mask(self.corrupted_B)
        self.template_B = self.add_post_mask(self.template_B)

    def fit_in_B(self, lmax, nside):
        self.corrupted_B = self.add_mask(hp.alm2map(hp.map2alm(self.QU_B1, lmax=lmax)[2], nside=nside))
        self.template_B = self.add_mask(hp.alm2map(hp.map2alm(self.QU_B2, lmax=lmax)[2], nside=nside))
        coeffs = np.polyfit(self.template_B[np.nonzero(self.template_B)], self.corrupted_B[np.nonzero(self.corrupted_B)], 1)
        slope, intercept = coeffs
        print(f"Slope: {slope}, Intercept: {intercept}")
        self.cleaned_B = self.add_post_mask(self.corrupted_B - slope * self.template_B)
        self.corrupted_B = self.add_post_mask(self.corrupted_B)
...
obj = EBCorrection(cmb, lmax, nside, mask=bin_mask, post_mask=bin_mask, fit_flag='QU')
obj.run_EBcorrection()
...
# np.save(f'./testnew/B/{number}/corrupted_B.npy',corrupted_B)
# np.save(f'./testnew/B/{number}/cleaned_B.npy',cleaned_B)
