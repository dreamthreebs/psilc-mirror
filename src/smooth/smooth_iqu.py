...
def check_maps_IQU(maps_pos_list:list, lmax:int, nside:int)->None:
    for file in maps_pos_list:
        ...
        T = hp.alm2map(TEBalm[0], nside=nside)
        E = hp.alm2map(TEBalm[1], nside=nside)
        B = hp.alm2map(TEBalm[2], nside=nside)
        TEB = np.vstack([T, E, B])

        for index, (IQUmap_type,TEBmap_type) in enumerate(zip("IQU","TEB")):
            # hp.mollview(IQU[index], title=f'{IQUmap_type}');plt.show()
            hp.orthview(hp.pixelfunc.ma(IQU[index],badval=0),half_sky=True, rot=[100,50,0],cmap='viridis', title=f'{IQUmap_type}');plt.show()
...
def smooth2iqu(bl_temp_arr, bl_grad_arr, bl_curl_arr, bl_std_temp, bl_std_grad, bl_std_curl, m_cmb_files=None, m_fg_files=None, m_noise_files=None, m_nstd_files=None, bin_mask=None, apo_mask=None, nside_in=512, nside=512, num_sim=None, save_path=None)->None:
    ...
    for index, (m_cmb, m_fg, m_noise, m_nstd) in enumerate(zip_longest(m_cmb_files, m_fg_files, m_noise_files,m_nstd_files, fillvalue=None)):
        ...
        if bin_mask is not None:
            sm_iqu = hp.alm2map([alm_temp, alm_grad, alm_curl], nside=nside) * bin_mask
        else:
            sm_iqu = hp.alm2map([alm_temp, alm_grad, alm_curl], nside=nside)
        ...
        # else:
        np.save(f'{save_path}/{freq}.npy', sm_iqu)
...
def split_iqu(path):
    maps = glob.glob(os.path.join(path, '*.npy'))
    map_list = sorted(maps, key=lambda x: int(Path(x).stem))
    I_list=[];Q_list=[];U_list=[]
    for file in map_list:
        freq = int(Path(file).stem)
        print(f'split{freq}...')
        m = np.load(file)
        I_list.append(m[0]);Q_list.append(m[1]);U_list.append(m[2])

    I=np.array(I_list);Q=np.array(Q_list);U=np.array(U_list)
    I_PATH = os.path.join(path,'I')
    Q_PATH = os.path.join(path,'Q')
    U_PATH = os.path.join(path,'U')
    paths = [(I_PATH, I), (Q_PATH, Q), (U_PATH, U)]
...
def data_creater(root_path, cmb_data_list=None, fg_data_list=None, nstd_data_list=None, bin_mask=None, apo_mask=None):
    ...
    if not os.path.exists(CMBPATH):
        os.makedirs(CMBPATH)
    smooth2iqu(bl_temp_arr, bl_grad_arr, bl_curl_arr, bl_std_temp, bl_std_grad, bl_std_curl, m_cmb_files=cmb_data_list, m_fg_files=None,m_nstd_files=None, save_path=CMBPATH, bin_mask=bin_mask, apo_mask=apo_mask)
    split_iqu(CMBPATH)
    ...
    if not os.path.exists(SIMPATH):
        os.makedirs(SIMPATH)
    smooth2iqu(bl_temp_arr, bl_grad_arr, bl_curl_arr, bl_std_temp, bl_std_grad, bl_std_curl, m_cmb_files=cmb_data_list, m_fg_files=fg_data_list,m_nstd_files=nstd_data_list, save_path=SIMPATH, bin_mask=bin_mask,apo_mask=apo_mask)
    split_iqu(SIMPATH)
    ...
    if not os.path.exists(FGPATH):
        os.makedirs(FGPATH)
    smooth2iqu(bl_temp_arr, bl_grad_arr, bl_curl_arr, bl_std_temp, bl_std_grad, bl_std_curl, m_cmb_files=None, m_fg_files=fg_data_list,m_nstd_files=None, save_path=FGPATH, bin_mask=bin_mask, apo_mask=apo_mask)
    split_iqu(FGPATH)
    ...
    smooth2iqu(bl_temp_arr, bl_grad_arr, bl_curl_arr, bl_std_temp, bl_std_grad, bl_std_curl, m_cmb_files=None, m_fg_files=None,m_nstd_files=nstd_data_list, save_path=NOISEPATH, bin_mask=bin_mask, apo_mask=apo_mask)
    split_iqu(NOISEPATH)
    ...
    smooth2iqu(bl_temp_arr, bl_grad_arr, bl_curl_arr, bl_std_temp, bl_std_grad, bl_std_curl, m_cmb_files=None, m_fg_files=fg_data_list,m_nstd_files=nstd_data_list, save_path=FGNOISEPATH, bin_mask=bin_mask, apo_mask=apo_mask)
    split_iqu(FGNOISEPATH)

    print('create cmbfg...')
    CMBFGPATH = os.path.join(root_path, 'CMBFG')
    if not os.path.exists(CMBFGPATH):
        os.makedirs(CMBFGPATH)
    smooth2iqu(bl_temp_arr, bl_grad_arr, bl_curl_arr, bl_std_temp, bl_std_grad, bl_std_curl, m_cmb_files=cmb_data_list, m_fg_files=fg_data_list,m_nstd_files=None, save_path=CMBFGPATH, bin_mask=bin_mask, apo_mask=apo_mask)
    split_iqu(CMBFGPATH)
...
def noise_simulator(root_path, nstd_data_list, sim_min=0, sim_max=100):
    ...
    for i in range(sim_min, sim_max):
        ...
        if not os.path.exists(NOISEPATH):
            os.makedirs(NOISEPATH)
        smooth2iqu(bl_temp_arr, bl_grad_arr, bl_curl_arr, bl_std_temp, bl_std_grad, bl_std_curl, m_cmb_files=None, m_fg_files=None,m_nstd_files=nstd_data_list, save_path=NOISEPATH,num_sim=i)
        split_iqu(NOISEPATH)
...
if __name__=="__main__":
    ...
    # cmbfg = glob.glob('./TEST_PART/noPS_northNOI2048/CMB/*.npy')
