...
def smooth_b(dataframe, bl_std_curl, bl_curl, m_list, bin_mask, apo_mask, lmax, nside, save_path):
    smooth_list = []
...
if __name__=='__main__':
    ...
    bl_curl = bl_curl_creater(lmax=lmax)

    cmb_list = sorted(glob.glob(f'../eblc/eblc_data/sim/*.npy'), key=lambda x: int(Path(x).stem))
    bin_mask = np.load('../mask/north/BINMASKG.npy')
    smooth_b(dataframe=df, bl_std_curl=bl_std_curl, bl_curl=bl_curl, m_list=cmb_list, bin_mask=bin_mask, apo_mask=None, lmax=lmax, nside=nside, save_path='./eblc_data/smsim')
