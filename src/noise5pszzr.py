...
if __name__ == '__main__':
    ...
    for i in range(n_sim_start, n_sim_end):
        print(f'eblc at loop:{i}')
        eblc_at_diff_freq(lmax_eblc=350, nside_eblc=512, glob_path=f'./sim/NSIDE512BAND5/NOISESIM/{i}', save_path_eblc=f'../newdata/band5ps350/eblc/NOISESIM/{i}', method='zzr', path_bin_mask_eblc='./mask/north/BINMASKG.npy')

    for i in range(n_sim_start, n_sim_end):
        print(f'smooth at loop:{i}')
        smooth_eblc_result(lmax_sm=350, nside_sm=512, beam_base=63, path_df='../FGSim/FreqBand5', glob_path=f'../newdata/band5ps350/eblc/NOISESIM/{i}', save_path_sm=f'../newdata/band5ps350/SMNOISESIM/{i}', path_bin_mask_sm='./mask/north/BINMASKG.npy')

    for i in range(n_sim_start, n_sim_end):
        print(f'nilc at loop:{i}')
        nilc_noisepp(lmax_nilc=350, nside_nilc=512, Rtol=1/1000, path_apo_mask='./mask/north/APOMASKC1_5.npy', path_bin_mask='./mask/north/BINMASKG.npy', path_noise=f'../newdata/band5ps350/SMNOISESIM/{i}/data.npy', needlet_config=f'./eblcilc/needlets/needlet3.csv', load_path_weight='../newdata/band5ps350/simnilc/weight0.npz', save_path_nilc=f'../newdata/band5ps350/NOISENILC', number=i)

    for i in range(n_sim_start, n_sim_end):
        print(f'hilc:{i = }')
        hilc_noisepp(lmax_hilc=350, nside_hilc=512, path_apo_mask='./mask/north/APOMASKC1_5.npy', path_bin_mask='./mask/north/BINMASKG.npy', path_noise=f'../newdata/band5ps350/SMNOISESIM/{i}/data.npy', load_path_wl='../newdata/band5ps350/simhilc/wl.npy', save_path_hilc='../newdata/band5ps350/NOISEHILC', number=i)

    for i in range(n_sim_start, n_sim_end):
        print(f'pilc:{i = }')
        pilc_noisepp(lmax_pilc=350, nside_pilc=512, path_apo_mask='./mask/north/APOMASKC1_5.npy', path_bin_mask='./mask/north/BINMASKG.npy', path_noise=f'../newdata/band5ps350/SMNOISESIM/{i}/data.npy', load_path_w='../newdata/band5ps350/simpilc/w.npy', save_path_pilc='../newdata/band5ps350/NOISEPILC', number=i)
