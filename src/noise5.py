...
if __name__ == '__main__':
    ...
    for i in range(n_sim_start, n_sim_end):
        smooth_eblc_result(lmax_sm=500, nside_sm=512, beam_base=9, path_df='../FGSim/FreqBand5', glob_path=f'../data/noapo/eblc/NOISESIM/{i}', save_path_sm=f'../data/noapo/SMNOISESIM/{i}', path_bin_mask_sm='./mask/north/BINMASKG.npy')

    for i in range(n_sim_start, n_sim_end):
        nilc_noisepp(lmax_nilc=500, nside_nilc=512, Rtol=1/1000, path_apo_mask='./mask/north/BINMASKG.npy', path_bin_mask='./mask/north/BINMASKG.npy', path_noise=f'../data/noapo/SMNOISESIM/{i}/data.npy', needlet_config=f'./eblcilc/needlets/needlet2.csv', load_path_weight='../data/noapo/simnilc/weight0.npz', save_path_nilc=f'../data/noapo/NOISENILC', number=i)

    for i in range(n_sim_start, n_sim_end):
        print(f'{i = }')
        hilc_noisepp(lmax_hilc=500, nside_hilc=512, path_apo_mask='./mask/north/BINMASKG.npy', path_bin_mask='./mask/north/BINMASKG.npy', path_noise=f'../data/noapo/SMNOISESIM/{i}/data.npy', load_path_wl='../data/noapo/simhilc/wl.npy', save_path_hilc='../data/noapo/NOISEHILC', number=i)

    for i in range(n_sim_start, n_sim_end):
        print(f'{i = }')
        pilc_noisepp(lmax_pilc=500, nside_pilc=512, path_apo_mask='./mask/north/BINMASKG.npy', path_bin_mask='./mask/north/BINMASKG.npy', path_noise=f'../data/noapo/SMNOISESIM/{i}/data.npy', load_path_w='../data/noapo/simpilc/w.npy', save_path_pilc='../data/noapo/NOISEPILC', number=i)
