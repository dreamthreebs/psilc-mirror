...
bl = hp.gauss_beam(fwhm=np.deg2rad(63)/60, lmax=lmax, pol=True)[:,2]

mb = hp.alm2map(hp.almxfl(hp.map2alm(m_cmb, lmax=lmax)[2], bl/bl_220), nside=nside)
...
diff_m = m[4]-mb*bin_mask
hp.orthview(diff_m, rot=[100,50,0], half_sky=True)
plt.show()

l = np.arange(lmax+1)
mask = np.load('../../../src/mask/north/APOMASKC1_10.npy')
