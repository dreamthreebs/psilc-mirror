...
for n_rlz in range(n_rlz_begin, n_rlz_end):
    ...
    noise_low = np.load(f'../{nside}/NOISE_LOW/{freq}/{n_rlz}.npy')
    ...
    PSNOISE_LOW_path = Path(f'./{nside}/PSNOISE_LOW/{freq}')
    PSNOISE_LOW_path.mkdir(parents=True, exist_ok=True)
    np.save(f'./{nside}/PSNOISE_LOW/{freq}/{n_rlz}.npy', psnoise_low)
